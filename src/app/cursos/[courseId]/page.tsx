...
export default function AdminEditLessonPage({ params }: { params: { courseId: string } }) {
  ...
  const handleDelete = async (lessonId: string) => {
    if (window.confirm("¿Estás seguro de que quieres eliminar esta lección?")) {
      // --- OPCIONAL: Aquí también podrías borrar los archivos de Storage asociados a esta lección ---
      // Por ahora, solo borra el documento de Firestore como en tu código original.
      try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // --- INICIO DE CÓDIGO ACTUALIZADO (BORRAR ARCHIVOS ANTIGUOS) ---

      // 1. Lógica para borrar el VIDEO antiguo si se sube uno nuevo
      if (editingLesson && videoFile && editingLesson.videoUrl) {
        try {
          // Obtenemos la referencia del archivo antiguo desde su URL
          const oldVideoRef = ref(storage, editingLesson.videoUrl);
          // Borramos el archivo
          await deleteObject(oldVideoRef);
        } catch (deleteError) {
          console.warn("No se pudo borrar el video antiguo (quizás ya no existía):", deleteError);
          // No detenemos el proceso, continuamos para subir el nuevo
        }
      }

      // 2. Lógica para borrar el MATERIAL DE APOYO antiguo si se sube uno nuevo
      if (editingLesson && supportFile && editingLesson.supportMaterialUrl) {
        try {
          const oldSupportRef = ref(storage, editingLesson.supportMaterialUrl);
          await deleteObject(oldSupportRef);
        } catch (deleteError) {
          console.warn("No se pudo borrar el material de apoyo antiguo:", deleteError);
        }
      }
      // --- FIN DE CÓDIGO ACTUALIZADO ---

      // Código original para subir los archivos nuevos
      if (videoFile) {
